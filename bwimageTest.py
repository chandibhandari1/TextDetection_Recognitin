...
for (x1,y1,x2,y2) in final_boxes:
    ...
    #rect= cv2.rectangle(img_copy,(x1,y1),(x2,y2),(0,255,0),2)
    #cv2.imwrite("bwimageOut_noText.jpg", rect)
    img_crop = Image.fromarray(img[y1-1: y2+1, x1-1:x2+1])
    text = pytesseract.image_to_string(img_crop, config='--psm 8').strip()
    att_text = cv2.putText(img_copy, text, (x1,y1), 0, .7, (0, 0, 255), 2)
    cv2.imwrite("bwimageOut_withText.jpg", att_text)
